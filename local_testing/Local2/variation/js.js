(function () {
    try {
        /* main variables */
        var debug = 1;
        var variation_name = "TravelShift_T54-1";
        ...
        /* Variation Init */
        // console.log("Outsidetest54_1Activate")
        function init() {
            // Check if the current pathname exists in the array
        // console.log("test54_1Activate")
            if (
                (window.location.href.includes('https://guidetoiceland.is/book-trips-holiday') ||
                    window.location.href.includes('https://guidetoiceland.is/book-trips-holiday/holidays-vacation-packages')) && // Tours or Packages
                (window._travelshift.page === 'tourSearch' || window._travelshift.page === 'tourCategory' || window._travelshift.page === '') // Search or Category
            ) {
                ...
                if (!window.observerCS2Test54_1) {
                    window.observerCS2Test54_1 = true;
        ...
        window.addEventListener('travelshift:routeChangeComplete', e => {
            // console.log("test54_1")
            init()
        });
        function CS2Test54_1bserver(list, observer) {
        ...
        if (window._travelshift && window._travelshift.chunksLoadingData && window._travelshift.chunksLoadingData.status != "loaded") {
            ...
            const observer = new PerformanceObserver(CS2Test54_1bserver);
