        ...
        function init() {
            ...
            document.querySelectorAll('[class*="pro-list-results-front-door"] a[data-test="pro-list-result"]').forEach(card => {
                ...
                if (card.textContent.includes('Instant Book availability')) {
                  // Check if the <li> with the appropriate text exists, either for desktop or mobile
                  const existingLi = [...(ul ? ul.querySelectorAll('li') : [])].some(li => li.textContent.includes('Instant Book availability') || li.textContent.includes('Instant Book avail.'));
              
                  if (!document.querySelector('.tt_Available')) {
                      // Use different text for desktop and mobile based on screen width
                      let text;
                      if (window.innerWidth > 699) {
                          text = 'Instant Book available';  // For desktop
                      } else {
                          text = 'Instant Book avail.';        // For mobile
                      }
                      
                      // Append the <li> only if it's not already added
                      createAndAppendLi(text);
                  }
              
                  // Add the 'hidetext' class to the parent div of the first div containing the text
                  const selectedElement = card.querySelector('.dn.m_db.mt2');
                  const parentDiv = selectedElement ? selectedElement.closest('div') : null;
                  if (parentDiv && !parentDiv.classList.contains('hidetext')) {
                      parentDiv.classList.add('hidetext');
                  }
              
                  // Find the .black.flex.items-center parent and add the 'Instant_hideitem' class
                  const parentElement = card.querySelector('.black.flex.items-center');
                  if (parentElement && !parentElement.classList.contains('Instant_hideitem')) {
