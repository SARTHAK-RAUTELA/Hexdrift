(function () {
    try {
      ...
      /**
       * Wait for an element to appear in the DOM and trigger a callback.
       */
      ...
      /**
       * Live event listener (event delegation).
       */
      ...
      /**
       * Add a class to the body based on the author's name.
       */
      function addBodyClassByAuthor() {
        var targetEl = document.querySelector(
          '[class*="composable-customer-header_root"] + .mb6 > div:nth-child(4) [class*="Type_text2"]'
        );
        if (!targetEl) return;
  
        var author = targetEl.textContent.trim().replace(/^By\s+/i, "");
        var allowedNames = [
          "Kristy Snyder",
          "Melanie Fourie",
          "Nicki Escudero",
        ...
        if (allowedNames.indexOf(author) > -1) {
          var className = author.replace(/\s+/g, "_");
          document.body.classList.add(className);
        }
      }
  
      /**
       * Move Table of Contents for mobile devices.
       */
      function moveTocMobile() {
        if (window.innerWidth > 1023) return;
  
        var tocList = document.querySelector('#Table\\ of\\ Contents + ul');
        var target = document.querySelector(
          '[class*="composable-customer-header_root"] + .mb6 > div:nth-child(4)'
        );
      ...
      /**
       * Insert banner image if the current URL matches.
       */
      function insertBannerImage() {
        var currentPath = window.location.pathname;
  
        for (var i = 0; i < DATA_OBJECT.length; i++) {
          var item = DATA_OBJECT[i];
          if (item.url.indexOf(currentPath) > -1) {
            if (!document.querySelector(".banner_img")) {
              var hero = document.querySelector('[class*="cobaltized-cost-page-hero_root"]');
              if (hero) {
                hero.insertAdjacentHTML(
                  "afterend",
                  '<div class="banner_img"><img src="' + item.img + '" alt="Banner Image"/></div>'
                );
              }
            }
          }
      ...
      /**
       * Wrap extra divs and custom TOC inside a main wrapper.
       */
      function wrapExtraDivsAndToc() {
        var toc = document.querySelector('#Table\\ of\\ Contents');
        var container = document.querySelector("div.mb6");
        if (!toc || !container) return;
      
        var allDivs = Array.prototype.slice.call(container.querySelectorAll(":scope > div"));
        if (allDivs.length <= 4) return;
      
        if (!container.querySelector(".main-wrapper")) {
          ...
          for (var j = 4; j < allDivs.length; j++) {
            var el = allDivs[j];
            var skipSelector = ".bb.b-gray-300.bg-white.pt5.pb5.m_pt6.m_pb6";
            if (!(el.matches && el.matches(skipSelector)) && el.id !== "hireapro") {
              customWrap.appendChild(el);
            }
          }
      
          var tocList = toc.nextElementSibling ? toc.nextElementSibling.cloneNode(true) : null;
          if (tocList) {
            ...
            para.textContent = "Find local pros for your project";
            tocDiv.appendChild(para);
      
            // ✅ CTA add
            var cta = document.createElement("a");
            cta.href = "#"; 
            cta.className = "ttcta-btn";
            cta.textContent = "Search for pros"; 
            tocDiv.appendChild(cta);
      
            mainWrapper.appendChild(customWrap);
            mainWrapper.appendChild(tocDiv);
      
            var fourthDiv = allDivs[3];
      ...
      function init() {
        ...
        document.addEventListener("click", function (e) {
            if (e.target.closest(".ttcta-btn")) {  
                e.preventDefault();
              var targetLink = document.querySelector('[class*="cost-page-cost-data_content"] + a , [class*="cobaltized-cost-page-hero_stickyHeader"] a');
              if (targetLink) {
                targetLink.click(); 
              }
            }
          ...
          (function () {
            const mapping = [
              {
                name: "Daniel Jones",
                fullText: "Expert reviewed by Daniel Jones, owner of Junk Miners in Carlsbad, CA.",
                linkText: "Junk Miners",
                url: "https://www.thumbtack.com/ca/carlsbad/junk-removal/junk-miners/service/275863999598453779"
              },
              {
                name: "Chris Davaa",
                fullText: "Expert reviewed by Chris Davaa, owner of 24 Seven Moving in Los Angeles, CA.",
                linkText: "24 Seven Moving",
                url: "https://thumbtack.com/ca/los-angeles/movers/24-seven-moving/service/431466909043875851"
              },
              {
                name: "Shawn Myers",
                fullText: "Expert reviewed by Shawn Myers, owner of Skilled Pugs Pressure Wash & Gutter Cleaning in Cartersville, GA.",
                linkText: "Skilled Pugs Pressure Wash & Gutter Cleaning",
                url: "https://www.thumbtack.com/ga/cartersville/pressure-washing/skilled-pugs-pressure-wash-gutter-cleaning/service/485915601361354785"
              },
              {
                name: "Jesus Cuevas",
                fullText: "Expert reviewed by Jesus Cuevas, owner of Bridge Branch Services in Houston, TX.",
                linkText: "Bridge Branch Services",
                url: "https://www.thumbtack.com/tx/houston/animal-removal-services/bridge-branch-services/service/462344869856665610"
              },
             
            ];
          
            const checkEl = document.querySelector('[class*="composable-customer-header_root"] + .mb6 > div:nth-child(4) div p');
            const updateEl = document.querySelector('[class*="composable-customer-header_root"] + .mb6 > div:nth-child(4) div + div p');
            ...
            if (checkEl && updateEl) {
              ...
              mapping.forEach(item => {
                if (text.includes(item.name)) {
                  const linkedText = item.fullText.replace(
                    item.linkText,
                    `<a href="${item.url}" target="_blank">${item.linkText}</a>`
                  );
                  updateEl.innerHTML = linkedText;
                }
              });
            }
          })();
      ...
      function onHydrate(list, observer) {
      ...
      if (!window.isHydrated) {
        ...
        var observer = new PerformanceObserver(onHydrate);
