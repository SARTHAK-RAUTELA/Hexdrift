(function () {
    try {
        const debug = 1;
        const variation_name = "cre-t-76";

        /* Mini jQuery-like utility */
        var _$;
        !(function (factory) {
            _$ = factory();
        })(function () {
            ...
            bm.prototype = {
                ...
                log(...args) {
                    if (debug && console && console.log) {
                        console.log(variation_name, ...args);
                    }
                },
                addClass(classNames) {
      ...
      <div class="${variation_name}-text-container">
        <div class="${variation_name}-text-wrapper">
          <div class="${variation_name}-text-content">
            <h1 class="sc-88b5dd01-0 cfyWof ${variation_name}-custom-h1">
              Dine <span class="cre-t-76-mobile-same-line">off-peak &amp;</span>
              <span class="cre-t-76-mobile-save">&nbsp;save 50%&nbsp;</span>
              on your entire meal
            </h1>
            <p class="sc-e5a2a45e-0 dabarf ${variation_name}-custom-p">
      ...
      <div class="${variation_name}-bottom-container">
          ...
          <div class="${variation_name}-bottom-content">
            ...
            <div class="${variation_name}-trusted-container">
              <div class="${variation_name}-profiles">
                <div class="${variation_name}-profile ${variation_name}-profile-1"><img src="${config.profile1}" alt="User 1" /></div>
                <div class="${variation_name}-profile ${variation_name}-profile-2"><img src="${config.profile2}" alt="User 2" /></div>
                <div class="${variation_name}-profile ${variation_name}-profile-3"><img src="${config.profile3}" alt="User 3" /></div>
              </div>
              <span class="${variation_name}-trusted-text">Trusted by 2.2M+ diners</span>
            </div>
          </div>
        </div>
      </div>`;
        ...
        function changeSubCopies() {
            const updates = [
                {
                    selector: '[data-attribute="booking_details_find_restaurant"] p',
                    text: "From fine dining to everyday favourites—browse top-rated restaurants near you and discover new cuisines.",
                },
                {
                    selector: '[data-attribute="booking_details_date_time"] p',
                    text: "Pay a small booking fee to secure your discounted table. Your reservation is confirmed instantly.",
                },
                {
                    selector: '[data-attribute="booking_details_First_table"] p',
                    text: "Save 50% on your entire food bill while enjoying the same great quality and service as full-paying customers.",
                },
            ];

            updates.forEach(({ selector, text }) => {
                helper.waitForElement(selector, () => {
        ...
        function init() {
            ...
            const intervalId = setInterval(() => {
                ...
                if (!mainForm || !mainForm.parentElement) return;

                if (!document.querySelector(`.${variation_name}-text-container`)) {
                    mainForm.parentElement.insertAdjacentHTML("beforebegin", textHtml);
                }

                if (!document.querySelector(`.${variation_name}-bottom-container`)) {
                    mainForm.parentElement.insertAdjacentHTML("afterend", bottomHtml);
                }

                const submitBtn = mainForm.querySelector('button[type="submit"]');
                if (submitBtn && submitBtn.parentElement) {
                    const btnParent = submitBtn.parentElement;
                    btnParent.classList.add(`${variation_name}-form-button-parent`);

                    if (!document.querySelector(`.${variation_name}-curved-container`)) {
                        const parent = btnParent.parentElement;
                        if (parent) parent.classList.add(`${variation_name}-curved-parent`);
                        btnParent.insertAdjacentHTML("afterend", curvedHtml);
                    }
                }
            }, 250);
