...
`;
        ...
        function init() {
            ...
            if (!document.querySelector(".reviewsection.disktop")){
        ...
        function thumbtackTest144(list, observer) {
            ...
            console.log(list.getEntries(), "list.getEntries()")
            list.getEntries().forEach((entry) => {
                console.log(entry, "entry")
                if (entry.entryType === "mark" && entry.name === "afterHydrate") {
                    observer.disconnect();
                    clearInterval(test144Interval)
                    // Trigger the Variation code here-----
        ...
        if (!window.isHydrated) {
            ...
            console.log("gvbhnj")
            // Observe the changes  ------- PerformanceObserver
            const observer = new PerformanceObserver(thumbtackTest144);
            observer.observe({ entryTypes: ["mark"] });
        } else {
            //simple wait for Element-----------
            waitForElement("body", init, 50, 15000);
        }
