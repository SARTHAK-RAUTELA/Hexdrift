(function () {
    try {
      ...
      /**
       * Resets the search functionality by clearing search highlights and toggling buttons.
       */
      ...
      /**
       * Displays search results and highlights matching terms.
       * Updates the search message and toggles buttons.
       * 
       * @function displaySearchResult
       */
      function displaySearchResult() {
        resetSearch();
        const searchInput = document.querySelector(".cre-t-36-search-input");
        const textContent = searchInput.value.trim().toLowerCase();
        console.log("Search Input Value:", textContent); // Debugging
  
        if (textContent.length > 0) {
          const texts = textContent.split(" ");
          const searchMessage = document.querySelector(".cre-t-36-search-message");
          const mainContainer = document.querySelector(".cre-t-36-main-list-container");
  
          if (mainContainer) {
            const selector = texts.map(text => `[data-cre-text*="${text}"]`).join(",");
            const matchedElements = mainContainer.querySelectorAll(selector);
            console.log("Matched Elements:", matchedElements); // Debugging
  
            document.body.classList.add("cre-t-36-searched");
            document.querySelector(".cre-t-36-search-button").classList.add("cre-t-36-hide");
            document.querySelector(".cre-t-36-search-reset").classList.remove("cre-t-36-hide");
            ...
            if (matchedElements.length > 0) {
              matchedElements.forEach(item => {
                ...
                // Add matching classes to parent containers
                let parent = item.closest(".cre-t-36-list-container");
                while (parent) {
                  parent.classList.add("cre_search_matched");
              ...
              searchMessage.innerHTML = `No files matched for '<span>${textContent}</span>'`;
            }
          }
        }
      }
      ...
      function eventListeners() {
        ...
        live(".cre-t-36-folder-title-container", 'click', function (event) {
          ...
          if (document.body.classList.contains("cre-t-36-searched")) {
