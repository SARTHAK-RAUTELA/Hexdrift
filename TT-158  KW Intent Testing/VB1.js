...
`;
    ...
    function mostreview() {
      function getUndefinedPNumber(card) {
        ...
        return -1; // Return -1 if the element is not found or no review count exists
      }
      let cardsArray = [];
      document.querySelectorAll('[class*="pro-list-results-front-door"] [class*="pro-list-results-front-door"]:first-child > div.ph3 > div').forEach(function (card) {
        // Check if the card contains the specified selector

        if (card.querySelector('[data-test="pro-list-result"]')) {
          const reviewNumber = getUndefinedPNumber(card);
          // Push all cards to the array, even those without reviews (they get -1)
          if (reviewNumber) {
            cardsArray.push({
              card,
              reviewNumber
      ...
      // Sort the cards by reviewNumber (cards with -1 go to the bottom)
      cardsArray.sort(function (a, b) {
        return b.reviewNumber - a.reviewNumber;
      });
      // Apply the order classes based on the sorted positions
      cardsArray.forEach(function (item, index) {
        item.card.classList.forEach(className => {
          if (className.startsWith('order-')) {
            item.card.classList.remove(className); // Remove existing order classes
          }
        });
        item.card.classList.add(`order-${index + 1}`); // Add the new order class
      });
    }
    ...
    function mostrating() {
      ...
      function getGreenPNumber(card) {
        ...
        return -1; // Return -1 if no rating is found (to push the card to the bottom)
      }
      ...
      document.querySelectorAll('[class*="pro-list-results-front-door"] > div > div > div.ph3 > div').forEach(function (card, index) {
        ...
        if (card.querySelector('[data-test="pro-list-result"]')) {
          ...
          // Push all cards to the array, even those without ratings (ratingNumber will be -1 for them)
          cardsArray.push({
            card,
            ratingNumber,
            reviewCount
