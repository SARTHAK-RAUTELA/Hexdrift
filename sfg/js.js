(function () {
    try {
      ...
      // 		".welcomeContainer .sms-heading",
      // 		function () {
      // 			var smsHeading = document.querySelector(
      // 				".welcomeContainer .sms-heading"
      // 			);
      // 			if (smsHeading) {
      ...
      function creFourBubble() {
        ...
        var regex = /(\d{1,3})%/;
        var matchResult = text.match(regex);
        if (matchResult) {
          var extractedNumbers = parseInt(matchResult[1], 10);
          var creDealFour =
      ...
      function ifly3() {
        var creSuperSaver =
          ...
          '                  <p class="cre-card-text">AVOID THE CROWDS AND SAVE ~<span class="cre-dataPercentage">30</span>% IN OFF-PEAK TIMES!</p>' +
          ...
          document.querySelectorAll(".btn.btn-add").forEach(function (e) {
            ...
            if (e.getAttribute("data-name") && e.getAttribute("data-price")) {
              ...
              if (dataName.indexOf("2 Flights Super Saver") !== -1) {
                ...
                parent.classList.add("cre_two_Flights");
              }
              addClassToParent(
      ...
      function init() {
        ...
        if (document.querySelector("body.cre-t-6")) {
          ...
          // 	function () {
          // 		document.querySelector(".cre-textItem .cre-card-text").innerText = "Avoid the crowds and save ~30% in off-peak times!";
          // 		helper.waitForElement(".welcomeContainer .sms-heading",
          // 			function () {
          ...
          // 					locationPerPerson.forEach(function (location) {
          // 						if (text.includes(location.tunnel)) {
          // 							if (locationsToUpdate.includes(location.tunnel)) {
      ...
      function calculateDiscount() {
        // Add class to the closest parent element for each matching product
    

        helper.waitForElement(".cre_3_2_flight .price-value, .cre-superSaver .cre-super-price p, .updatePrice", function () {
            var productElements = document.querySelectorAll(".product-name");
            for (var i = 0; i < productElements.length; i++) {
              if (productElements[i].textContent === "2 Flights") {
                var closestDiv = productElements[i].closest(".product");
                if (closestDiv) {
                  if (!document.querySelector(".cre_3_2_flight")) {
                    closestDiv.classList.add("cre_3_2_flight");
                  }
                }
              }
            }
        ...
        // helper.waitForElement(".cre_3_2_flight .price-value, .cre-superSaver .cre-super-price p",function () {
            setTimeout(function() {
                function getDiscountPercentage() {
                    var mainPriceElement = document.querySelector(".cre_3_2_flight .price-value ");
                    var offerPriceElement = document.querySelector(".cre-superSaver .cre-super-price p");
        
                    console.log(offerPriceElement);
                    if (!mainPriceElement || !offerPriceElement) {
                      return 0;
                    }
                    ...
                    var mainPrice = extractNumber(mainPriceElement.textContent);
                    var offerPrice = extractNumber(offerPriceElement.textContent);
        
                    if (isNaN(mainPrice) || isNaN(offerPrice) || mainPrice === 0) {
                      return 0;
                    }
                    var discount = mainPrice - offerPrice;
                    // return (discount / mainPrice) * 100;
                    var actualPrice = (discount / mainPrice) * 100 
                    var formattedDiscount = actualPrice.toFixed(2);
      
        
                    if (formattedDiscount > 28) {
                      var roundedDiscount = Math.round(formattedDiscount * 1.04)
                    } else {
                      var roundedDiscount = Math.round(formattedDiscount)
                    }
                  ...
                  // Example usage:
                  var setPrice = getDiscountPercentage().toString()
                  document.querySelector(".cre-dataPercentage").textContent = setPrice;
        ...
        // Calculate the discount price dynamically
        // helper.waitForElement(".cre_3_2_flight .price-value, .cre-superSaver .cre-super-price p",function () {
            setTimeout(function() {
                ...
                document.querySelector(".cre-usp-bar .updatePrice").textContent = calculateSavePrice();
            }, 3000);
      ...
      // Call the dynamic function calculateSavePrice
      helper.waitForElement(".pageContent, .cre-card-tex",function () {
          calculateDiscount();
      },50,50000);
